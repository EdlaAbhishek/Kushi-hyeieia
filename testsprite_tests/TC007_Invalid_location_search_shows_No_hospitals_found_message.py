...
async def run_test():
    ...
    try:
        ...
        # -> Click the cookie consent 'Accept & Continue' button (index 3) to dismiss the banner so sign-in can proceed.
        ...
        # -> Click the 'Hospitals' link in the top navigation (index 182).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div[2]/header/div/nav/a[3]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type 'zzzz-not-a-real-place-123' into the location search input (index 1048) and submit the search (press Enter).
        frame = context.pages[-1]
        # Input text
